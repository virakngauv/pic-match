...
export class GameServer {
  ...
  claim(token: string, claim: MatchClaimCommand, now = Date.now()) {
    return this.withRoom(claim.roomCode, (room) =>
      room.claim(token, claim, now),
    )
  }

  prepareRematch(token: string, roomCode: string, now = Date.now()) {
    return this.withRoom(roomCode, (room) => room.prepareRematch(token, now))
  }
